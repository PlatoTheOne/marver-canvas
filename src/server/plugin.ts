  ...
  return {
    ...
    load(id) {
      ...
      if (id === '\0' + VIRTUAL_CONFIG) {
        ...
        return `export default ${JSON.stringify({ locale: config.locale, viewports: config.viewports, themes: config.themes, zoomSpeed: config.zoomSpeed, noTheme: themeFile() == null, setup: setupPending, projectName: config.share.name || basename(root), branding: config.share.branding })}`
      }
